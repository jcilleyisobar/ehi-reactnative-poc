...
public class MemberInfoViewModel extends ManagersAccessViewModel {
    ...
    private EHIContactProfile getUpdatedContactProfile() {
        ...
        for (EHIPhone number : numbers) {
            ...
            } else if (!TextUtils.isEmpty(number.getMaskPhoneNumber())) {
                number.setPhoneNumber(number.getMaskPhoneNumber());
                notEmptyNumbers.add(number);
            }
        }
    ...
    public boolean isMainPhoneNumberEmpty() {
        final EHIPhone phone = mContactProfile.getValue().getPhone(0);
        return EHITextUtils.isEmpty(phone.getPhoneNumber()) && EHITextUtils.isEmpty(phone.getMaskPhoneNumber());
    }
