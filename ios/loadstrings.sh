...
generate_plist_keys () {
	...
	do
		# appropriate line
		found=$(echo "$json" | grep -o "\"$plistKey\":\".*\"")
...
do
	json=$(curl https://enterprise-xqa1-aem.enterprise.com/content/eapp/${locale}/jcr:content.json?dd=cachebust3)
	
	echo $json > Enterprise/Localizations/localizable.${locale}.json
