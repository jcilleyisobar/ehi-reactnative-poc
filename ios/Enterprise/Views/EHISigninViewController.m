...
{
    BOOL hideUserCnotainer  = self.viewModel.layout == EHISigninLayoutEnrollment;
    BOOL hideBottomCntainer = self.viewModel.isReservationFlow ? self.viewModel.isReservationFlow : hideUserCnotainer;
    
    self.navigationHeightConstraint.isDisabled = YES;
    self.forgotNumberButtonHeight.isDisabled   = hideUserCnotainer;
    
    MASLayoutPriority userContainerPriority   = hideUserCnotainer  ? MASLayoutPriorityRequired : MASLayoutPriorityDefaultLow;
    MASLayoutPriority bottomContainerPriority = hideBottomCntainer ? MASLayoutPriorityRequired : MASLayoutPriorityDefaultLow;
    
    [self.usernameContainer mas_remakeConstraints:^(MASConstraintMaker *make) {
        make.height.equalTo(@0.0).priority(userContainerPriority);
    }];
    
    [self.bottomContainer mas_remakeConstraints:^(MASConstraintMaker *make) {
        make.height.equalTo(@0.0).priority(bottomContainerPriority);
    }];
}
