...
@interface  EHIDriverInfoViewModel ()
...
- (void)presentSignIn
{
    ...
    self.router.transition.present(EHIScreenMainSignin).handler(^{
        ...
        if(user != nil) {
            [welf fillFieldsWithUser:user];

            welf.didSignInDuringReservation = YES;
            welf.builder.reservation.hasToAssociate = YES;
...
- (BOOL)hideEmailOptIn
{
    BOOL wantsSpecialOffers = [EHIUser currentUser].preference.email.specialOffers == EHIOptionalBooleanTrue;
    return wantsSpecialOffers;
}
...
- (void)fillFieldsWithUser:(EHIUser *)user
{
